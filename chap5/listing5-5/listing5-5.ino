/* Using a random number generator to get a number between 1 and 99,
  the player guesses if the next number is less or greater than the first number. It uses the serial monitor, and three digital I/O pins which are activated when momentarily touched to a ground point such as the USB shield box.*/
int randomNum1;
int randomNum2;
...
void setup () {
  ...
  Serial.begin (9600);
  Serial.println ("Ground pin 7 to start ");//display message one time
}
void loop () {
...
rerun:
    ...
    //this section will not let you win
    Serial.println (" ");
